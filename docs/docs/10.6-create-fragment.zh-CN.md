...
在大多数情况下，你可以使用 `key` prop 在你从 `render` 返回的元素上指定 keys。然而，这在一个情况下会失败：如果你有两组你需要记录的子级，将没有办法在不使用包裹元素的情况下放置一个 key 在每组上。
...
被传递对象的 keys （即是，`left` 和 `right`）被用作整组子级的 keys，并且对象的 keys 的顺序被用于决定渲染的子级的顺序。通过这个改变，这两个子级将会不被卸载的恰当的在 DOM 里排序。
